...
def render(client):
    ...
    if len(fg_df) >= 2:
        ...
        col1.metric("市場情緒指數 (0 fear - 100 greed)", f"{latest_fg['fear_greed_score']:.1f} ({latest_fg['fear_greed_rating']})", f"{score_delta:+.1f}")
    elif len(fg_df) == 1:
        col1.metric("市場情緒指數 (0 fear - 100 greed)", f"{fg_df.iloc[0]['fear_greed_score']:.1f} ({fg_df.iloc[0]['fear_greed_rating']})")

    if not industry_df.empty:
        best, worst = industry_df.iloc[0], industry_df.iloc[-1]
        col2.metric("今日表現最佳產業", best["industry_name"], f"{best['daily_return_pct']:+.2f}%")
        col3.metric("今日表現最弱產業", worst["industry_name"], f"{worst['daily_return_pct']:+.2f}%")

    st.divider()

    if not industry_df.empty:
        fig_bar = go.Figure(go.Bar(
            x=industry_df["daily_return_pct"], y=industry_df["industry_name"], orientation="h",
            marker_color=["red" if v >= 0 else "green" for v in industry_df["daily_return_pct"]],
        ))
        fig_bar.update_layout(height=700, xaxis_title="漲跌幅 (%)", yaxis=dict(autorange="reversed"))
        st.plotly_chart(fig_bar, use_container_width=True)

        st.divider()
        st.subheader("各產業表現")
        sort_option = st.radio("排序方式", ["依表現排序", "依產業代碼排序"], horizontal=True)
        display_df = (
        ...
        for i in range(0, len(display_df), cols_per_row):
            ...
            for col, (_, row) in zip(row_cols, row_data.iterrows()):
                col.metric(row["industry_name"], f"{row['weighted_avg_price']:.2f}", f"{row['daily_return_pct']:+.2f}%")
    else:
