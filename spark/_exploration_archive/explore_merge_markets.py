...
def explore():
    ...
    #     .appName("stock-pulse-merge-explore")
    #     .master("local[*]")
    ...
    # merged = merge_markets(cleaned_twse, cleaned_tpex, twse_official_ids, tpex_official_ids)

    # print(f"合併後總筆數: {merged.count()}")
    # print(f"TWSE 筆數: {merged.filter(F.col('market') == 'TWSE').count()}")
    # print(f"TPEx 筆數: {merged.filter(F.col('market') == 'TPEx').count()}")

    # # 動態分區覆寫設定:只覆寫這次資料涵蓋的分區,不影響其他已寫入的歷史分區
    # spark.conf.set("spark.sql.sources.partitionOverwriteMode", "dynamic")

    # output_path = "gs://stock-pulse-data-lake/clean/stock_daily/"
    # # merged_with_date.coalesce(1).write.mode("overwrite").partitionBy("dt").parquet(output_path)
