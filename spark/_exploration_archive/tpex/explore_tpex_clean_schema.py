"""
[ARCHIVED] TPEx 原始 schema 驗證 + 清洗邏輯探索,含官方股票清單過濾後
「開高低收為 null 但均價有值」異常情況的稽核。

狀態: clean_tpex() 的邏輯已併入 spark/jobs/clean_stock.py,可正式使用。
此檔案保留給日後遇到類似資料異常時,直接取消註解重跑除錯,不用重新想一次怎麼查。

若要移動此檔案的位置,請確認下方 sys.path.append 的 .parent 層數
與新的資料夾深度一致(目前假設: 專案根目錄/_exploration_archived/tpex/)。

--------------------------------------------------------------------------
以下除錯區塊索引(全部預設關閉,用到時取消對應區塊的註解即可):

  clean_tpex() 內:
    [D1] 掃描所有數值欄位裡「無法解析成標準數字」的異常值
    [D2] 找出 close_price 為 '---' 的完整那一列(驗證異常值長相)

  explore() 內:
    [D3] 檢查 change_symbol_raw 有哪些非典型格式的相異值
    [D4] 對 D3 抓到的樣本逐字元印出 Unicode 編碼(揪出不可見字元,例如尾隨空白)
    [D5] 檢查其他數值欄位是否也有尾隨空白問題
    [D6] 清洗前資料快速預覽
    [D7] 清洗後資料快速預覽(對照 D6)
    [D8] 驗證「尾隨空白」異常值清洗後有沒有正常轉型(不應變成非預期的 null)
    [D9] 驗證特定股票(範例 '---' 案例)清洗後的欄位值是否如預期變成 null
    [D10] 統計「開高低收為 null、但均價有值」的筆數
    [D11] 檢查 D10 那群 stock_id 的長度分布(找格式規律)
    [D12] D11 的範例名稱列表
    [D13] 對照組: 開高低收「有值」的這群,stock_id 長度分布長怎樣
--------------------------------------------------------------------------
"""
...
def clean_tpex(df):
    ...
    # [D2] 用途: 找出 close_price 為 '---' 的完整那一列,確認異常值出現時其他欄位的狀態
...
def explore():
    ...
    # [D11] 用途: 檢查 D10 那群 stock_id 的長度分布(用來找格式規律,例如是否都是特殊代號)
    ...
    # [D12] 用途: D11 的範例名稱列表(前 15 筆),肉眼確認長相
    # print("\n=== 範例名稱(前 15 筆)===")
    # null_ohlc_ids.show(15, truncate=False)

    # [D13] 用途: 對照組,開高低收「有值」的這群,stock_id 長度分布長怎樣(跟 D11 比較)
