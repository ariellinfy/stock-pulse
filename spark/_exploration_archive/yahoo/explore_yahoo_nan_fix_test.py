...
raw_df = spark.read.option("multiline", "true").json("local_output/yahoo_sample/stock_id_7794.json")
...
nan_count = cleaned.filter(
    F.isnan(F.col("close")) | F.isnan(F.col("open")) | F.isnan(F.col("high")) | F.isnan(F.col("low"))
).count()
