...
def detect_clean_layer_gaps(spark, bucket_name: str, twse_industry_records: list[dict], gap_threshold: int = 10):
    ...
    expected_counts = [
        Row(dt=day, expected_count_adjusted=listing_dates.filter(F.col("listing_date") <= F.to_date(F.lit(day))).count())
    ...
    print(f"clean 層,gap > {gap_threshold} 的交易日數: {gaps.count()} / {result.count()}")
